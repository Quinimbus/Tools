...
public class QCollectorsTest {
    ...
    public void testOneOrNone() {
        ...
        assertEquals(
                "The stream contains more than one value",
                assertThrows(IllegalStateException.class, () -> Stream.of(1, 2).collect(oneOrNone()))
                        .getMessage());
    }
    ...
    public void testExactlyOne() {
        assertEquals(
                "The stream contains no value",
                assertThrows(IllegalStateException.class, () -> Stream.empty().collect(exactlyOne()))
                        .getMessage());
        assertEquals(1, Stream.of(1).collect(exactlyOne()));
        assertEquals(
                "The stream contains more than one value",
                assertThrows(IllegalStateException.class, () -> Stream.of(1, 2).collect(exactlyOne()))
                        .getMessage());
    }
}
